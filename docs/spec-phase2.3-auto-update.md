...
| D5  | `autoUpdater.autoDownload = false` + 显式 `downloadUpdate(token)`                               | v6 的 `autoDownload=true` 模式下没有中断下载的 API；改用 `CancellationToken`（来自 `builder-util-runtime`）才能让 skipVersion 真正中断进行中的下载                                                                                                                                                |
| D6  | `autoUpdater.autoInstallOnAppQuit = true`（默认）                                               | 用户不主动 Restart 也会在下次正常退出时安装；语义最自然，不打扰                                                                                                                                                                                                                                   |
| D7  | skipVersion 命中时 `token.cancel()` 并发 `skipped` 事件                                         | 已经开始下载的也能取消；store 记录后这个版本号不再提示，直到发布更高版本。下载 Promise 因 cancel 被 reject，由 update-available 内的 catch 吞掉（state 已是 skipped）                                                                                                                             |
...
| D13 | electron-updater 用 default import + 解构                                                       | electron-updater v6 是 CommonJS 包，`import { autoUpdater }` 在 ESM 项目运行时炸（typecheck 不报错因 TS esModuleInterop 假装可以）。必须 `import electronUpdater from 'electron-updater'; const { autoUpdater } = electronUpdater`。`UpdateCheckResult` 是 type-only，可继续用 inline `type` 标记 |
...
// electron-updater v6 是 CJS，必须 default import 后解构（D13）
